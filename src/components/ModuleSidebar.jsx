...
function ModulesSidebar() {
    ...
    return (
        <aside className="flex flex-col w-[40%] max-md:w-full top-0 z-50 sticky overflow-hidden mt-10">
            <div className="flex flex-col px-5 py-6 mx-auto  w-full text-base text-black rounded-3xl border border-solid bg-stone-50 border-slate-300 max-md:mt-10 overflow-hidden max-h-[60%]">
                ...
                <div className="flex flex-col overflow-y-auto max-h-[calc(100vh-200px)]">
                    {modules.map((module, index) => (
                        <div key={index} className={`flex gap-6 px-4 py-6 mt-${index === 0 ? '7' : '3'} whitespace-nowrap bg-white rounded-2xl border border-solid border-slate-300`}>
                            {module.icon}
